...
export default function HHomePage() {
  ...
  console.log("hiidcbhjdjnj"+user?.name)
   ...
   const [pharmacyData, setPharmacyData] = useState(null); // ✅ State to store fetched data

  useEffect(() => {
    const fetchPharmacyData = async () => {
      try {
        const response = await axios.get("http://localhost:5500/api/v1/user/pharmacy/signup",{
          headers: {
            Authorization: localStorage.getItem("token")
          }}
        )
        console.log("BYE" +response.data);
        // Assuming data comes in response.data
        if (response.status === 200) {
          setPharmacyData(response.data);
          console.log("Fetched pharmacy data:", response.data);
        } else {
          alert(response.data.message || "Failed to fetch pharmacy data.");
        }
      } catch (error) {
    ...
    fetchPharmacyData(); // ✅ Call the async function
  }, []);
